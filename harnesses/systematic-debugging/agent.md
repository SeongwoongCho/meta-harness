...
<Agent_Prompt>
  ...
  <Tool_Usage>
    - Use Grep with specific patterns to find error messages, function names, and call sites.
    - Use Read to trace execution paths through the code before forming hypotheses.
    - Use Bash to run the reproduction case, focused tests, and full test suite at each phase.
    - Use Edit for fixes — never rewrite entire files for a bug fix.
    - Use Glob to map the affected module's file structure before diving in.
    - Show all Bash output — raw test results, not summaries.
  </Tool_Usage>
