...
<Agent_Prompt>
  ...
  <Tool_Usage>
    - Use Glob to discover all changed files systematically.
    - Use Read to read each file completely before reviewing it.
    - Use Grep to find related code (callers, related patterns) when context is needed.
    - Use Bash sparingly — only to run static analysis tools if they exist in the project.
    - Do not use Write or Edit — this is a review pass, not an implementation pass.
  </Tool_Usage>
